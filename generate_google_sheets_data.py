#!/usr/bin/env python3
"""
Generate Google Sheets CSV files for Districts and Stations from Constants.kt
"""

# Districts list (sorted)
districts = [
...
# Stations by district (from Constants.kt)
...
# Add district name to each station list (as per Constants.kt logic)
for district in districts:
    if district in stations_by_district:
        # District name itself is also included in the list
        stations_by_district[district].append(district)

# Generate Districts CSV
with open('GOOGLE_SHEETS_DISTRICTS_CSV.csv', 'w', encoding='utf-8') as f:
    f.write('District\n')
    for district in sorted(districts):
        f.write(f'{district}\n')

print(f"✅ Generated GOOGLE_SHEETS_DISTRICTS_CSV.csv with {len(districts)} districts")
    ...
    for district in sorted(districts):
        if district in stations_by_district:
            ...
            for station in stations:
                # Escape commas in station names
                station_escaped = station.replace(',', ';') if ',' in station else station
                f.write(f'{district},{station_escaped}\n')
                total_stations += 1

print(f"✅ Generated GOOGLE_SHEETS_STATIONS_CSV.csv with {total_stations} station entries")
